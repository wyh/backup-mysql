...
if [ ! -z $match ]; then
    ...
    if [ $? -eq 0 ]; then 
        ...
        if [ $backups -gt 1 ]; then
            echo "More than  one backup found."
            echo "Going to delete one to save the space"
            rm -rf `ls -1|head -1`
        fi
        cd $backup_dir;
    fi
    ...
    innobackupex --user=bkpuser --password=test --incremental $backup_dir/incr/ --incremental-basedir=$fullbase_dir/$fullbackup_dir
    if [ $? -eq 0 ]; then
